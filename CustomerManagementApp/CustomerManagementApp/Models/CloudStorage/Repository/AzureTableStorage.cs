...
namespace Wolnik.Azure.TableStorage.Repository
{
    ...
    {
        private readonly CloudTableClient __Client;
        private ConcurrentDictionary<string, CloudTable> __Tables;

        /// <summary>
        /// Initializes a new instance of the <see cref="AzureTableStorage" /> class.
        /// </summary>
        /// <param name="connectionString">The connection string.</param>
        public AzureTableStorage(string connectionString) : this(connectionString, null)
        { }

        internal AzureTableStorage(string connectionString, ICloudStorageClient client)
        {
            __Client = client.CloudTableClient ?? CloudStorageAccount.Parse(connectionString).CreateCloudTableClient();
            __Tables = new ConcurrentDictionary<string, CloudTable>();
        }
        ...
        {
            var table = await EnsureTable(tableName).ConfigureAwait(false);
            PopulateRowKeyIfNotExists(entity);
        ...
        private static void PopulateRowKeyIfNotExists(ITableEntity entity)
        {
            if (string.IsNullOrWhiteSpace(entity.RowKey))
            {
                entity.RowKey = (DateTime.MaxValue.Ticks - DateTime.UtcNow.Ticks).ToString("d20");
            }
        }

        public async Task<object> DeleteAsync(string tableName,string partitionKey, string rowKey)
        {
            var entity =   await GetAsync<ITableEntity>(tableName, partitionKey, rowKey);
            return DeleteAsync(tableName, entity);
        }
        /// <summary>
        /// Deletes the entity.
        /// </summary>
        ...
        private async Task<CloudTable> EnsureTable(string tableName)
        {
            ...
            {
                var table = __Client.GetTableReference(tableName);
                await table.CreateIfNotExistsAsync().ConfigureAwait(false);
                __Tables[tableName] = table;
            }

            return __Tables[tableName];
        }
